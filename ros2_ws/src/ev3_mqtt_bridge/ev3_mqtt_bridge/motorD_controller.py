...
class MotorDController(Node):
    """
    MotorD Controller (Action 版)
    ...
    def __init__(self) -> None:
        ...
        self.get_logger().info(
            f'[MotorDController] connect action: {self.get_namespace()}/{self.action_name}, '
    ...
    def demo_tick(self):
        ...
        # Step 1: abs +45 * ratio
            ...
            if not self.sent:
                motor_deg = self._deg(180.0)
                self.goal_future = self.send_goal('abs', motor_deg, 200)
        ...
        # Step 3: rel +45 * ratio
            ...
            if not self.sent:
                motor_deg = self._deg(360)
                self.goal_future = self.send_goal('rel', motor_deg, 200)
                self.goal_future.add_done_callback(self._on_result)
                self.sent = True
        ...
        # Step 4: home（回到 0）
            ...
            if not self.sent:
                ...
                self.goal_future = self.send_goal('home', 200, 0)
                self.goal_future.add_done_callback(self._on_result)
                self.sent = True
