...
class MotorAController(Node):
    ...
    def __init__(self) -> None:
        ...
        self.get_logger().info(
            f'[MotorAController] connect action: {self.get_namespace()}/{self.action_name}, '
    ...
    def demo_tick(self):
        ...
        # Step 1: abs +45 * ratio
            ...
            if not self.sent:
                motor_deg = self._deg(45.0)
                self.goal_future = self.send_goal('abs', motor_deg, 300)
        ...
        # Step 3: rel +45 * ratio
            ...
            if not self.sent:
                motor_deg = self._deg(90)
                self.goal_future = self.send_goal('rel', motor_deg, 300)
                self.goal_future.add_done_callback(self._on_result)
                self.sent = True
        ...
        # Step 4: home（回到 0）
            ...
            if not self.sent:
                ...
                self.goal_future = self.send_goal('home', 300, 0)
                self.goal_future.add_done_callback(self._on_result)
                self.sent = True
