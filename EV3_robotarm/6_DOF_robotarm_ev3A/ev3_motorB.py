...
# ⚠️ 建議：如果你之後用 launch 一次開 A/B，可以把這行註解掉，改在 launch 裡只執行一次
...
# 與電腦端/橋接一致的 topic（EV3A / motorB）
TOPIC_CMD    = "ev3A/motorB/cmd"
TOPIC_STATUS = "ev3A/motorB/status"

# 這台 EV3 的「本馬達 ID」；只有當收到 m{MY_ID}: 前綴或 無前綴(廣播) 才會執行
MY_ID = 2

# 把 motor_id 對應到 EV3 實體輸出口（這支是 2 -> outB）
ID_TO_PORT = {
    2: "outB",
}
