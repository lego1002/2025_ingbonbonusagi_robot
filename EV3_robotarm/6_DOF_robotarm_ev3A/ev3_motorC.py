...
# === Motor C Topics ===
TOPIC_CMD    = "ev3A/motorC/cmd"
TOPIC_STATUS = "ev3A/motorC/status"
...
def on_connect(client, userdata, flags, rc):
    if rc == 0:
        ...
        client.publish(
            TOPIC_STATUS,
            "EV3 motor node online (id={}, port={})".format(MY_ID, ID_TO_PORT.get(MY_ID, "?")),
            qos=0,
            retain=False
        )
    ...
    try:
        if low == "run":
            ...
            if len(parts) >= 2:
                ...
                spd = 300 if len(parts) < 3 else int(float(parts[2]))
            ...
            parts = low.split()
            spd = 300 if len(parts) < 2 else int(float(parts[1]))
